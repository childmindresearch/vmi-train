...
public class Movable : MonoBehaviour
{
    ...
    public void Update() {
        if (lastPosition != transform.position || lastRotation != transform.eulerAngles) {
            DataCaptureSystem.Instance.ReportEvent($"{this.name}.transform.position", transform.position);
            DataCaptureSystem.Instance.ReportEvent($"{this.name}.transform.eulerAngles", transform.eulerAngles);
        }
        lastPosition = transform.position;
        lastRotation = transform.eulerAngles;
    }
